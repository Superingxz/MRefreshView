...
import com.jsonmo.xrefreshview.PullToRefreshBase;
import com.jsonmo.xrefreshview.PullToRefreshBase.OnRefreshListener;
...
public class PullRefreshListViewActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mPullListView = new PullToRefreshListView(this){
            @Override
            protected void addFooterLoadingLayout(LoadingLayout footerlayout) {
                new MFooterLoadingLayout(PullRefreshListViewActivity.this);
            }
        };

        setContentView(mPullListView);
        
        mPullListView.setPullLoadEnabled(true);	//上拉加载不可用
        mPullListView.setScrollLoadEnabled(false);	//上拉自动加载可用
   
        mCurIndex = mLoadDataCount;
        mListItems = new LinkedList<String>();
        mListItems.addAll(Arrays.asList(mStrings).subList(0, mCurIndex));	//不可改变的list
